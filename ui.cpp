...
RecoveryUI::RecoveryUI()
  ...
  char aosp_version[PROPERTY_VALUE_MAX];
  property_get("ro.build.version.release", aosp_version, "(unknown)");
  char lineage_version[PROPERTY_VALUE_MAX];
  property_get("ro.lineage.version", lineage_version, "(unknown)");
  version_ = std::string("Android ") + aosp_version + " / " +
             std::string("LineageOS ") + lineage_version;
...
RecoveryUI::InputEvent RecoveryUI::WaitInputEvent() {
  ...
  do {
    ...
    while (event_queue_len == 0 && rc != ETIMEDOUT) {
      rc = pthread_cond_timedwait(&event_queue_cond, &event_queue_mutex, &timeout);
    }
