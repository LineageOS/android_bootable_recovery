...
static void choose_recovery_file(Device* device) {
  ...
  while (true) {
    ...
    if (chosen_item == Device::kGoHome || chosen_item == Device::kGoBack || chosen_item == entries.size() - 1) {
      break;
    }
    ...
    switch (chosen_action) {
      ...
      case Device::ENTER_RESCUE: {
        ...
        if (chosen_action == Device::ENTER_RESCUE) {
          ...
          status = ApplyFromAdb(device, false /* rescue_mode */, &reboot_action, ask_to_continue_unverified_fn);
        } else {
          adb = false;
