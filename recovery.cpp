...
static const char* time_off_3_data = "/data/vendor/time/ats_1";
static const char* time_off_4_data = "/mnt/vendor/persist/time/ats_1";
static const char* time_off_5_data = "/persist/time/ats_1";
...
static void copy_userdata_files() {
  if (ensure_path_mounted("/data") == 0) {
    ...
    if (access(time_off_root, F_OK) != 0) {
      ...
      } else if (access(time_off_3_data, R_OK) == 0) {
  ...
  if (ensure_path_mounted("/mnt/vendor/persist") == 0) {
    if (access(time_off_4_data, R_OK) == 0) {
      file_copy(time_off_4_data, time_off_root);
    }
    ensure_path_unmounted("/mnt/vendor/persist");
  }

  if (ensure_path_mounted("/persist") == 0) {
    if (access(time_off_5_data, R_OK) == 0) {
      file_copy(time_off_5_data, time_off_root);
    }
