...
void Device::RemoveMenuItemForAction(Device::BuiltinAction action) {
  // We put actions that might be removed in the advanced menu only
  g_advanced_actions.erase(
      std::remove_if(g_advanced_actions.begin(), g_advanced_actions.end(),
                     [action](const auto& entry) { return entry.second == action; }));
  CHECK(!g_advanced_actions.empty());
}
