...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
  ...
  int vertical_real_estate = ScreenHeight() - margin_height_ * 2 -
                             gr_get_height(lineage_logo_.get()) -
                             TextLineHeight() * title_lines_.size();
  int menu_rows = vertical_real_estate / MenuItemHeight() - text_headers.size();
