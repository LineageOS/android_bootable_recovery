...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
  ...
  int max_help_message_height = MenuItemPadding() + TextLineHeight() * 2;
  int max_height = ScreenHeight() - margin_height_ * 2 - gr_get_height(lineage_logo_.get()) -
                   TextLineHeight() * title_lines_.size() - max_help_message_height;
  int menu_rows = max_height / MenuItemHeight() - text_headers.size();
