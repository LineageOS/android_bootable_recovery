...
TextMenu::TextMenu(bool scrollable, size_t max_items, size_t max_length,
  ...
  LOG(INFO) << "Made menu with max items " << max_display_items_;
...
int MenuDrawFunctions::DrawTextLine(int x, int y, const std::string& line, bool bold) const {
  int padding = menu_char_height_ * 2/3;
  gr_text(gr_menu_font(), x, y + padding, line.c_str(), bold);
  return padding + menu_char_height_ + padding;
}
...
int MenuDrawFunctions::DrawWrappedTextLines(int x, int y, const std::vector<std::string>& lines) const {
  ...
  size_t text_cols = (gr_fb_width() - x * 2) / menu_char_width_;
  int offset = 0;
  for (const auto& line : lines) {
    size_t next_start = 0;
...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
                                                   ...
                                                   size_t initial_selection) const {
  int menu_char_width = menu_draw_funcs_->menu_char_width_;
  int menu_char_height = menu_draw_funcs_->menu_char_height_;
  int menu_item_padding = menu_char_height * 2/3;
