...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
  ...
  int menu_rows = (ScreenHeight() - margin_height_*2 - gr_get_height(lineage_logo_.get())
                  - MenuItemPadding()) / MenuItemHeight() - text_headers.size();
  int menu_cols = (ScreenWidth() - margin_width_*2 - kMenuIndent) / MenuCharWidth();
  ...
  return std::make_unique<TextMenu>(wrap_selection, menu_rows, menu_cols, text_headers, text_items,
                                    initial_selection, MenuCharHeight(), *menu_draw_funcs_);
}
