...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    ...
    const int menu_item_height = MenuItemHeight() + MenuItemSpacing();

    if (point.y() >= menu_start_y_ &&
        point.y() < menu_start_y_ + menu_->ItemsCount() * menu_item_height) {
      int old_sel = menu_->selection();
      int relative_sel = (point.y() - menu_start_y_) / menu_item_height;
