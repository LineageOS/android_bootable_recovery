...
void ScreenRecoveryUI::draw_text_menu_locked(int& y) {
  ...
  for (int i = 0; i < menu_items && y < gr_fb_height(); ++i) {
    if (i == menu_sel) {
      ...
      DrawHighlightBar(0, y - 2, gr_fb_width(), 3 * menu_char_height_);
      // Bold white text for the selected item.
      SetColor(MENU_SEL_FG);
      y += menu_char_height_;
      gr_text(gr_menu_font(), x, y, menu_[i].text.c_str(), true);
      y += menu_char_height_;
      y += menu_char_height_;
      SetColor(MENU);
    } else {
      y += menu_char_height_;
      gr_text(gr_menu_font(), x, y, menu_[i].text.c_str(), false);
      y += menu_char_height_;
      y += menu_char_height_;
    }
  }
