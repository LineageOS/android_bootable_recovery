...
static const menu_item MAIN_MENU_ITEMS[] = {
  { "Reboot",           "ic_reboot",            "ic_reboot_sel" },
  { "Apply update",     "ic_system_update",     "ic_system_update_sel" },
  { "Factory reset",    "ic_factory_reset",     "ic_factory_reset_sel" },
  { "Advanced",         "ic_options_advanced",  "ic_options_advanced_sel" },
  { nullptr, nullptr }
};
...
Device::Device(RecoveryUI* ui) :
  ui_(ui),
  menu_(MAIN_MENU_TYPE, true, MAIN_MENU_ITEMS),
...
Device::BuiltinAction Device::InvokeMenuItem(int menu_position) {
  if (menu_position < 0) {
    if (menu_position == Device::kGoBack ||
      ...
      menu_.set(MAIN_MENU_TYPE, true, MAIN_MENU_ITEMS);
      menu_actions_ = MAIN_MENU_ACTIONS;
    }
    return NO_ACTION;
  }
  ...
  if (action == Device::ADVANCED_MENU) {
    menu_.set(ADVANCED_MENU_TYPE, false, ADVANCED_MENU_ITEMS);
