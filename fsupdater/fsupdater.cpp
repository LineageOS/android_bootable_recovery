...
static std::string updater_mount_context = "";
...
static bool blkroset(const std::string& device, bool ro) {
  int val = (ro ? 1 : 0);
  int fd;
  int rc;
...
static Value* SlotmountFn(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
  ...
  if (options == "ro") {
    ...
    if (!blkroset(device, false)) {
      return ErrorAbort(state, kVendorFailure, "%s() blkroset failed for %s",
          name, device.c_str());
    }
  }

  std::string mnt_options;
  if (!updater_mount_context.empty()) {
    mnt_options = "context=" + updater_mount_context;
  }

  if (mount(device.c_str(), mntpoint.c_str(), fstype.c_str(),
            flags, mnt_options.c_str()) != 0) {
