...
static bool umsToggleOptions(Device* device, std::map<std::string, bool>& options) {
  ...
  bool cdrom = options["cdrom"];
  bool nofua = options["nofua"];
  bool removable = options["removable"];
  bool ro = options["ro"];
  ...
  for (const auto& [key, value] : options) {
    if (!android::base::SetProperty("ums." + key, value ? "1" : "0")) {
      LOG(ERROR) << "Failed to set UMS property: " << key;
      return false;
    }
  }
...
static void umsStart(Device* device, std::string path) {
  ...
  if (!android::base::SetProperty("ums.file", path)) {
    LOG(ERROR) << "Failed to set UMS file property";
    goto out;
  }
