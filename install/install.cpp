...
static bool CheckAbSpecificMetadata(const std::map<std::string, std::string>& metadata, RecoveryUI* ui) {
  ...
  if (undeclared_downgrade && !(ui->IsTextVisible() && ask_to_continue_downgrade(ui->GetDevice()))) {
    return false;
  }
...
bool CheckPackageMetadata(const std::map<std::string, std::string>& metadata, OtaType ota_type, RecoveryUI* ui) {
