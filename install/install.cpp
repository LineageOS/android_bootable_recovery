...
/**
 * Checks for the existence of known kernel nodes that indicate hardware
 * anti-rollback protection is active, making downgrades unsafe.
 *
 * @return True if a prohibitive node is found, false otherwise.
 */
static bool is_downgrade_prohibited_by_hardware() {
    const std::vector<const char*> anti_rollback_nodes = {
        "/sys/kernel/boot_control/blow_ar",
    };

    for (const char* path : anti_rollback_nodes) {
        if (access(path, F_OK) == 0) {
            LOG(WARNING) << "Hardware anti-rollback node found at " << path
...
static InstallResult TryUpdateBinary(Package* package, bool* wipe_cache,
  ...
  if (!is_downgrade_prohibited_by_hardware()) {
