...
static void write_fstab_entry(const FstabEntry& entry, FILE* file) {
  if (entry.fs_type != "mtd" && entry.fs_type != "emmc" &&
      entry.fs_type != "bml" && !entry.fs_mgr_flags.vold_managed &&
      !entry.blk_device.empty() && entry.blk_device[0] == '/' &&
      !entry.mount_point.empty() && entry.mount_point[0] == '/') {
...
int ensure_volume_unmounted(const std::string& blk_device) {
  ...
  for (auto& entry : mounted_fstab) {
    if (entry.blk_device == blk_device) {
      ...
      if (result == -1) {
        LOG(ERROR) << "Failed to unmount " << blk_device << " from " << entry.mount_point << ": " << errno;
        return -1;
      }
    }
  }
